...
from .compose_engine import ComposeEngine, SUPPORTED_IMAGE_EXTS, CancelledException
...
class ComposeWorker(QThread):
    ...
    def run(self):
        try:
            ...
            result = engine.compose(self.files, self.output_path)
...
class ComposeDropList(QListWidget):
...
class ComposePage(QWidget):
    ...
    IMAGE_FILTER = "图片文件 (*.jpg *.jpeg *.png *.bmp *.gif *.tiff *.tif *.webp)"
    ...
    def _init_ui(self):
        ...
        title = QLabel("图片合成 PDF")
        title.setStyleSheet("font-size: 18px; font-weight: bold; margin: 5px 0;")
        layout.addWidget(title)

        info = QLabel("支持格式: JPG / PNG / BMP / GIF / TIFF / WebP  —  每张图片为一页 PDF")
        ...
        left_layout.addWidget(QLabel("图片列表（可拖拽排序）:"))
        self.file_list = ComposeDropList()
        left_layout.addWidget(self.file_list)
        self.summary_label = QLabel("已选择 0 张图片")
        ...
        self.btn_compose = QPushButton("开始合成")
        ...
        action_layout.addWidget(self.btn_compose)
    ...
    def _add_batch(self, label: str, files: list, batch_type: str = "add"):
        bid = self._next_batch_id
        self._next_batch_id += 1
        self._batches.append({"id": bid, "label": label, "files": list(files), "type": batch_type})
    ...
    def _redo_batch(self, batch_id: int):
        ...
        if batch["type"] == "delete":
            # 重新删除条目
            removed_paths = {item_data["file_path"] for item_data in batch["files"]}
            i = 0
            while i < self.file_list.count():
        ...
        self._rebuild_history_ui()
        self._update_summary()
        if self.file_list.count() > 0 and not self.output_path.text():
    ...
    def _add_files(self):
        files, _ = QFileDialog.getOpenFileNames(
            self, "选择图片", "", self.IMAGE_FILTER
        )
        if not files:
            return
        added = self._add_files_to_list(files)
        if added:
            self._add_batch(f"图片 × {len(added)}", added)
    ...
    def _add_folder(self):
        ...
        if not files:
            QMessageBox.information(self, "提示", f"文件夹中未找到支持的图片文件")
            return
        natural_sort(files)
    ...
    def _remove_selected(self):
        ...
        self._update_summary()
        if removed_items:
            self._add_batch(f"删除选中 × {len(removed_items)}", removed_items, batch_type="delete")
    ...
    def _auto_set_output_path(self):
        ...
        self.output_path.setText(os.path.join(first_dir, f"合成结果_{timestamp}.pdf"))

    def _browse_output(self):
        files = self.file_list.get_ordered_files()
        default_dir = os.path.dirname(files[0]) if files else ""
        path, _ = QFileDialog.getSaveFileName(
            self, "保存合成的 PDF", default_dir, "PDF 文件 (*.pdf)"
        )
        if path:
    ...
    def _update_summary(self):
        ...
        self.summary_label.setText(f"已选择 {count} 张图片")
        self.btn_compose.setEnabled(count > 0 and bool(self.output_path.text()))
    ...
    def _start_compose(self):
        ...
        self.worker = ComposeWorker(files, output_path)
    ...
    def _on_error(self, error_msg: str):
        self._set_ui_enabled(True)
        self.progress_bar.setVisible(False)
        if "操作已取消" in error_msg or "操作已被取消" in error_msg:
            self.status_label.setText("操作已取消")
            return
